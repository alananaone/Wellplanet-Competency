...
<body class="min-h-screen flex flex-col antialiased">
      ...
      <div class="screen-only-view space-y-7">
        ...
        <div class="bg-[#FFFDF9] rounded-2xl p-6 border border-[#E2DDD5] soft-card-shadow flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div class="flex items-center gap-3">
            <button onclick="openSelfCompetencyModal(currentSubReviewMember)" id="btn-sub-review-modal-trigger" class="inline-flex items-center gap-2 px-4 py-2.5 text-xs sm:text-sm font-semibold rounded-xl bg-[#E4ECD3] text-[#2D5239] hover:bg-[#D4DFC0] transition border border-[#CDE0BC] shadow-2xs">
              <i data-lucide="edit-3" class="w-4 h-4 text-[#557A61]"></i>
              <span id="btn-sub-review-modal-trigger-text">登記自評等級</span>
            </button>
  ...
  <script>
    ...
    // ACCURATE COMPETENCIES FOR EACH ROLE (REMOVED '其他創造的好事與預防的壞事')
    ...
    function renderSubReviewSection() {
      ...
      // Update Modal Trigger Button Text
      const modalTriggerText = document.getElementById('btn-sub-review-modal-trigger-text');
      if (modalTriggerText) {
        modalTriggerText.innerText = `登記【${currentSubReviewMember}】自評等級`;
      }
