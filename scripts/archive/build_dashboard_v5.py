...
<body class="min-h-screen flex flex-col antialiased">
            ...
            <div id="dropdownMenu" class="hidden absolute right-0 mt-2 w-88 origin-top-right rounded-2xl bg-[#FFFDF9] p-3 shadow-2xl ring-1 ring-black/5 z-50 divide-y divide-[#EFEAE1] border border-[#E8E2D8]">
              ...
              <div class="py-2">
                ...
                <button onclick="exportFullWorkbookClientSide()" class="w-full text-left flex items-center gap-3 px-3 py-2.5 text-xs sm:text-sm font-bold text-[#557A61] hover:bg-[#E4ECD3]/40 rounded-xl transition">
                  ...
                  <div>
                    <div>下載完整 Excel (全 Sheets 整合＋配色)</div>
  ...
  <script>
    ...
    // ==========================================
    // CLIENT-SIDE XLSX EXPORT VIA EXCELJS (100% PRESERVES FILLS, COLORS, BORDERS & DROPDOWNS)
    // ==========================================
    ...
    function buildSupervisorSheet(wb, supervisorName, memberNames, allEntries) {
    ...
    function buildSupervisorSummarySheet(wb, allEntries) {
      const ws = wb.addWorksheet("評主管總表", { views: [{ showGridLines: true }] });
      ws.mergeCells(1, 1, 1, 23);
      ws.getCell(1, 1).value = "好好星球文化基金會 360 年中成長評估 - 【評主管】回覆總表";
      styleRange(ws, 1, 1, 1, 23, { name: '微軟正黑體', size: 13, bold: true, color: { argb: 'FF3E2723' } }, COLOR_PINK_BLUSH, { horizontal: 'left', vertical: 'middle', indent: 1 });
      ...
      styleRange(ws, 2, 1, 2, supHeaders.length, fontSubHeader, COLOR_PEACH_CREAM, alignHeader);
      ws.getRow(2).height = 26;

      let rIdx = 3;
      allEntries.filter(e => e.relation === "主管").forEach(e => {
        const se = e.supervisor_eval || {};
        const rowVals = [
        ...
        rowVals.forEach((val, i) => {
          ws.getCell(rIdx, i + 1).value = val;
        });
        styleRange(ws, rIdx, 1, rIdx, rowVals.length, fontBody, COLOR_WHITE, alignLeft);
        for (let c = 1; c <= 19; c++) {
          ws.getCell(rIdx, c).alignment = alignCenter;
        }
        ws.getRow(rIdx).height = 40;
        rIdx++;
      });

      for (let c = 1; c <= supHeaders.length; c++) {
        ws.getColumn(c).width = c <= 19 ? 15 : 34;
      }
    }

    function buildPeerSummarySheet(wb, allEntries) {
      const ws = wb.addWorksheet("評同事總表", { views: [{ showGridLines: true }] });
      ws.mergeCells(1, 1, 1, 19);
      ws.getCell(1, 1).value = "好好星球文化基金會 360 年中成長評估 - 【評同事】回覆總表";
      styleRange(ws, 1, 1, 1, 19, { name: '微軟正黑體', size: 13, bold: true, color: { argb: 'FF3E2723' } }, COLOR_PINK_BLUSH, { horizontal: 'left', vertical: 'middle', indent: 1 });
      ws.getRow(1).height = 30;
      ...
      styleRange(ws, 2, 1, 2, peerHeaders.length, fontSubHeader, COLOR_PEACH_CREAM, alignHeader);
      ws.getRow(2).height = 26;

      let rIdx = 3;
      allEntries.filter(e => e.relation === "同事").forEach(e => {
        const pe = e.peer_eval || {};
        const rowVals = [
          e.timestamp, e.email, e.target,
      ...
      for (let c = 1; c <= peerHeaders.length; c++) {
        ws.getColumn(c).width = c <= 16 ? 15 : 34;
      }
    }
    ...
    async function exportSupervisorExcelClientSide(supName) {
      ...
      buildSupervisorSheet(wb, supName, memberNames, RAW_DATA);
      const buffer = await wb.xlsx.writeBuffer();
      const blob = new Blob([buffer], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
      saveAs(blob, `自評_${supName}主管專用_部屬自評彙整表.xlsx`);
      showToast(`已成功下載【${supName}】部屬自評 XLSX（全彩 Chunk 格式＋選單）！`);
    }

    async function exportFullWorkbookClientSide() {
      const wb = new ExcelJS.Workbook();
      Object.keys(SUPERVISOR_TEAMS).forEach(supKey => {
        buildSupervisorSheet(wb, supKey, SUPERVISOR_TEAMS[supKey], RAW_DATA);
      });
      buildSupervisorSummarySheet(wb, RAW_DATA);
      buildPeerSummarySheet(wb, RAW_DATA);
      const buffer = await wb.xlsx.writeBuffer();
      const blob = new Blob([buffer], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
      saveAs(blob, "好好星球_360年中成長評估_主管分流與完整彙整表.xlsx");
      showToast("已成功下載全組織完整 Excel 總表（全彩 Chunk 格式）！");
    }
...
print("Updated index.html: Integrated ExcelJS & FileSaver for 100% full-color Excel downloads directly in browser!")
