...
async function run() {
  ...
  Object.keys(SUPERVISOR_TEAMS).forEach(supKey => {
    buildSupervisorSheet(wb, supKey, SUPERVISOR_TEAMS[supKey], entries);
  });
  buildSupervisorSummarySheet(wb, entries);
  buildPeerSummarySheet(wb, entries);
  await wb.xlsx.writeFile("好好星球_360年中成長評估_主管分流與完整彙整表.xlsx");
  console.log("Exported full workbook successfully with full colors!");
}
