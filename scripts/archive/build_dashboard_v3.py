...
<body class="min-h-screen flex flex-col antialiased">
  ...
  <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-7">
    ...
    <section id="tab-section-self" class="tab-content space-y-7">
      ...
      <!-- RATING GUIDE CARD (NO 得分率 COLUMN) -->
      <div class="bg-[#FFFDF9] rounded-2xl border border-[#E8E2D8] soft-card-shadow overflow-hidden">
        <div class="chunk-subheader-banner px-6 py-4 flex items-center justify-between cursor-pointer hover:bg-[#FCE5CD]/90 transition" onclick="toggleRatingGuide()">
          <div class="flex items-center gap-3">
            ...
            <h3 class="text-sm sm:text-base font-bold text-[#4A2E1C] font-serif-tc">三、面向分數落點標準說明（Start / Grow / Keep / Good / Amazing!）</h3>
          </div>
          ...
          <table class="w-full text-xs sm:text-sm text-left border-collapse">
            <thead>
              <tr class="border-b border-[#E8E2D8] text-[#8C837C]">
                <th class="py-3 px-4 font-bold text-center w-20">等級 (Level)</th>
    ...
    <section id="tab-section-supervisor" class="tab-content hidden space-y-7">
      ...
      <div id="supervisor-feedback-list" class="space-y-5">
        <!-- Injected via JavaScript -->
      </div>
    </section>
    ...
    <section id="tab-section-peer" class="tab-content hidden space-y-7">
      ...
      <div id="peer-feedback-list" class="space-y-5">
        <!-- Injected via JavaScript -->
      </div>
    </section>
  ...
  <script>
    ...
    function getLevelSelectHtml(itemId, currentVal = '') {
      return `
        <div class="flex items-center gap-2.5 mt-3 pt-3 border-t border-[#E8E2D8]/80 flex-wrap">
          <span class="text-xs font-bold text-[#8C837C] flex items-center gap-1.5 shrink-0">
            <i data-lucide="check-square" class="w-3.5 h-3.5 text-[#557A61]"></i> 主管評核等級：
          </span>
          <select class="lv-select text-xs font-medium px-3.5 py-2 rounded-xl border border-[#D5CCC0] bg-[#FFFDF9] text-[#2E2827] focus:border-[#557A61] focus:ring-1 focus:ring-[#557A61] transition" data-item-id="${itemId}" onchange="handleLevelChange(this)">
            <option value="" ${currentVal === '' ? 'selected' : ''}>-- 請選取 Level --</option>
            <option value="L5" ${currentVal === 'L5' ? 'selected' : ''}>L5 (Amazing! · 遠超期待/標竿)</option>
            <option value="L4" ${currentVal === 'L4' ? 'selected' : ''}>L4 (Good · 優於期待/高標)</option>
            <option value="L3" ${currentVal === 'L3' ? 'selected' : ''}>L3 (Keep · 符合期待/穩定)</option>
            <option value="L2" ${currentVal === 'L2' ? 'selected' : ''}>L2 (Grow · 部分符合/建立中)</option>
            <option value="L1" ${currentVal === 'L1' ? 'selected' : ''}>L1 (Start · 發展起步/落差)</option>
          </select>
    ...
    function handleLevelChange(selectElem) {
      ...
      if (val === 'L5') {
        preview.innerHTML = `<span class="badge-l5 px-2.5 py-1 text-xs font-bold rounded-lg shadow-2xs">L5 · Amazing!</span>`;
      } else if (val === 'L4') {
        preview.innerHTML = `<span class="badge-l4 px-2.5 py-1 text-xs font-bold rounded-lg shadow-2xs">L4 · Good</span>`;
      } else if (val === 'L3') {
        preview.innerHTML = `<span class="badge-l3 px-2.5 py-1 text-xs font-bold rounded-lg shadow-2xs">L3 · Keep</span>`;
      } else if (val === 'L2') {
        preview.innerHTML = `<span class="badge-l2 px-2.5 py-1 text-xs font-bold rounded-lg shadow-2xs">L2 · Grow</span>`;
      } else if (val === 'L1') {
        preview.innerHTML = `<span class="badge-l1 px-2.5 py-1 text-xs font-bold rounded-lg shadow-2xs">L1 · Start</span>`;
      } else {
        preview.innerHTML = '';
      }
    }
    ...
    // ==========================================
    // TAB 1: SELF EVALUATION (CHUNK BY CHUNK WITH COMFORTABLE PADDING)
    // ==========================================
          ...
          html += `
            ...
            <div class="bg-[#FFFDF9] rounded-2xl border border-[#E8E2D8] soft-card-shadow overflow-hidden">
              ...
              <div class="chunk-header-banner px-6 py-5 flex flex-col md:flex-row md:items-center justify-between gap-3.5">
                ...
                <div class="text-xs font-medium text-[#7A4822] bg-white/80 px-4 py-2 rounded-xl border border-[#E6BDBD]/60">
              ...
              <div class="p-6 sm:p-8 space-y-7">
                ...
                <div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="bg-[#E4ECD3]/40 border border-[#CDE0BC] rounded-2xl p-5 sm:p-6 space-y-3">
                    ...
                    <div class="bg-[#FCE5CD]/40 border border-[#F5D3B3] rounded-2xl p-5 sm:p-6 space-y-3">
                      <div class="text-xs sm:text-sm font-bold text-[#8C4B1E] flex items-center gap-2">
                ...
                <div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <!-- 信任 -->
                    <div class="bg-[#EFEFF9]/50 border border-[#D5D5EB] rounded-2xl p-5 sm:p-6 flex flex-col justify-between space-y-3">
                      <div>
                        <div class="text-xs sm:text-sm font-bold text-[#4B4770] mb-2.5 flex items-center gap-2">
                          <span class="w-2.5 h-2.5 rounded-full bg-[#7B7299]"></span> 信任 (Trust)
                        </div>
                        <div class="bg-white/80 rounded-xl p-4 my-2 border border-[#D5D5EB]/60">
                    ...
                    <!-- 多元 -->
                    <div class="bg-[#E2F3F0]/50 border border-[#C4E6E1] rounded-2xl p-5 sm:p-6 flex flex-col justify-between space-y-3">
                      <div>
                        <div class="text-xs sm:text-sm font-bold text-[#2A5E59] mb-2.5 flex items-center gap-2">
                          <span class="w-2.5 h-2.5 rounded-full bg-[#437671]"></span> 多元 (Diversity)
                        </div>
                        <div class="bg-white/80 rounded-xl p-4 my-2 border border-[#C4E6E1]/60">
                    ...
                    <!-- 實驗 -->
                    <div class="bg-[#FFF4CD]/40 border border-[#FCE299] rounded-2xl p-5 sm:p-6 flex flex-col justify-between space-y-3">
                      <div>
                        <div class="text-xs sm:text-sm font-bold text-[#7A5E12] mb-2.5 flex items-center gap-2">
                          <span class="w-2.5 h-2.5 rounded-full bg-[#B88C22]"></span> 實驗 (Experiment)
                        </div>
                        <div class="bg-white/80 rounded-xl p-4 my-2 border border-[#FCE299]/60">
                    ...
                    <!-- 可持續 -->
                    <div class="bg-[#E4ECD3]/50 border border-[#CDE0BC] rounded-2xl p-5 sm:p-6 flex flex-col justify-between space-y-3">
                      <div>
                        <div class="text-xs sm:text-sm font-bold text-[#2D5239] mb-2.5 flex items-center gap-2">
                          <span class="w-2.5 h-2.5 rounded-full bg-[#557A61]"></span> 可持續 (Sustainability)
                        </div>
                        <div class="bg-white/80 rounded-xl p-4 my-2 border border-[#CDE0BC]/60">
                  ...
                  <div>
                    ...
                    <div class="space-y-4">
                      ${se.competencies.map((c, cIdx) => `
                        <div class="bg-[#FAF8F5] border border-[#E8E2D8] rounded-2xl p-5 sm:p-6 space-y-3.5 soft-card-shadow">
                          <div class="text-sm font-bold text-[#3E2723] font-serif-tc flex items-center gap-2">
                            <span class="w-2 h-2 rounded-full bg-[#557A61]"></span> ${c.title}
                          </div>
                          <div class="bg-white rounded-xl p-4.5 text-xs sm:text-sm text-[#2E2827] leading-relaxed whitespace-pre-line border border-[#EADBCE]/80 shadow-2xs">
                            ${c.answer || '（未填寫）'}
                          </div>
                  ...
                  <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                      ${Object.entries(se.reflection).map(([k, v], rIdx) => `
                        <div class="bg-[#FCE5CD]/30 border border-[#F3D1B0] rounded-2xl p-5 sm:p-6 flex flex-col justify-between space-y-3.5">
                          <div>
                            <div class="text-xs sm:text-sm font-bold text-[#783E16] mb-2 font-serif-tc">${k}</div>
                            <div class="bg-white/80 rounded-xl p-4 my-2 border border-[#F3D1B0]/60">
    ...
    function renderSupervisorSection() {
      ...
      feedbackContainer.innerHTML = filtered.map(e => {
        ...
        return `
          <div class="bg-[#FFFDF9] rounded-2xl border border-[#E8E2D8] p-6 sm:p-7 soft-card-shadow space-y-4.5">
            <div class="flex items-center justify-between border-b border-[#E8E2D8] pb-3.5">
              <div class="flex items-center gap-2.5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4.5 text-xs sm:text-sm">
              <div class="bg-[#F2EEE6]/60 p-4.5 rounded-2xl">
                <span class="font-bold text-[#557A61]">Q20. 願景使命理解之引導：</span>
                <p class="mt-2 text-[#4A433E] leading-relaxed">${se.q20_vision_mission || '（無）'}</p>
              </div>
              <div class="bg-[#F2EEE6]/60 p-4.5 rounded-2xl">
                <span class="font-bold text-[#8C4B1E]">Q21. 管理與文化精神建議：</span>
                <p class="mt-2 text-[#4A433E] leading-relaxed">${se.q21_improvement_advice || '（無）'}</p>
              </div>
              <div class="bg-[#F2EEE6]/60 p-4.5 rounded-2xl">
                <span class="font-bold text-[#2E2827]">Q22. 其他補充評價：</span>
                <p class="mt-2 text-[#4A433E] leading-relaxed">${se.q22_other_comments || '（無）'}</p>
              </div>
              <div class="bg-[#F4CCCC]/40 p-4.5 rounded-2xl border border-[#F4CCCC]">
                <span class="font-bold text-[#7A363A] flex items-center gap-2">
                  <i data-lucide="award" class="w-4 h-4"></i> Q23. 肯定與感謝的話（好好星光大賞）：
                </span>
                <p class="mt-2 text-[#592629] font-medium leading-relaxed">${se.q23_starlight_thanks || '（無）'}</p>
              </div>
            </div>
          </div>
        `;
      ...
      feedbackContainer.innerHTML = filtered.map(e => {
        ...
        return `
          <div class="bg-[#FFFDF9] rounded-2xl border border-[#E8E2D8] p-6 sm:p-7 soft-card-shadow space-y-4.5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4.5 text-xs sm:text-sm">
              <div class="bg-[#F2EEE6]/60 p-4.5 rounded-2xl">
                <span class="font-bold text-[#2E2827]">Q37. 工作與文化提升建議：</span>
                <p class="mt-2 text-[#4A433E] leading-relaxed">${pe.q37_improvement_advice || '（無）'}</p>
              </div>
              <div class="bg-[#F2EEE6]/60 p-4.5 rounded-2xl">
                <span class="font-bold text-[#2E2827]">Q38. 其他補充評價：</span>
                <p class="mt-2 text-[#4A433E] leading-relaxed">${pe.q38_other_comments || '（無）'}</p>
              </div>
              <div class="bg-[#F4CCCC]/40 p-4.5 rounded-2xl border border-[#F4CCCC]">
                <span class="font-bold text-[#7A363A] flex items-center gap-2">
                  <i data-lucide="award" class="w-4 h-4"></i> Q39. 肯定與感謝的話（好好星光大賞）：
                </span>
                <p class="mt-2 text-[#592629] font-medium leading-relaxed">${pe.q39_starlight_thanks || '（無）'}</p>
              </div>
            </div>
          </div>
        `;
          ...
          <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ${[...supEntries, ...peerEntries].filter(e => (e.supervisor_eval?.q23_starlight_thanks || e.peer_eval?.q39_starlight_thanks)).map(e => {
                ...
                return `
                  <div class="bg-[#F4CCCC]/30 border border-[#F4CCCC] rounded-2xl p-5 text-xs sm:text-sm text-[#592629] font-medium flex items-start gap-3.5">
    ...
    // ==========================================
    // CLIENT-SIDE XLSX CHUNK EXPORT (WITH RATING GUIDE DEFINITIONS & NO 得分率)
    // ==========================================
...
print("Updated index.html: generous padding inside all text boxes & removed 得分率 column!")
