...
RATING_GUIDE_ROWS = [
    ("L5", "Amazing!", "90% 以上", "遠超職位期待，表現為團隊之標竿與典範。", "讚賞其突出貢獻，探討經驗複製機制，在對話中轉為「帶其他人一起做」方向。", COLOR_L5_BG),
    ("L4", "Good", "80% 以上", "優於職位期待，持續展現高標準成果。", "肯定並具體指出哪些行為超出標準，設定具挑戰性的下一步目標。", COLOR_L4_BG),
    ("L3", "Keep", "60%–79%", "符合職位門檻，展現穩定的工作交付。", "確認穩定度，指出下一階可以往前之處，維持節奏並選一至兩項深化。", COLOR_L3_BG),
    ("L2", "Grow", "50%–59%", "部分符合，部分能力/行為仍在建立階段。", "說明落差所在，聚焦一項具體可練習的行為，納入下一期 IDP 設定指標。", COLOR_L2_BG),
    ("L1", "Start", "50% 以下", "目前的具體事證還看不到這項職能，或事證與職能要求落差明顯。", "明確對齊職位基本門檻與要求，提供即時支援與回饋引導。", COLOR_L1_BG),
]
...
def create_supervisor_chunk_sheet(ws, supervisor_name, member_names, all_entries):
    ...
    guide_title = ws.cell(2, 1, value="評分標準與面向分數落點說明（不對員工公布分數與總分，只回饋落點）")
    ...
    guide_headers = ["等級 (Level)", "落點名稱", "得分率", "定義說明", "回饋語氣與後續動作"]
    for col_i, h in enumerate(guide_headers, 1):
        cell = ws.cell(3, col_i, value=h)
    ...
    for idx, (lvl, name, score_rate, definition, action, bg_color) in enumerate(RATING_GUIDE_ROWS, 4):
        c1 = ws.cell(idx, 1, value=lvl)
        c2 = ws.cell(idx, 2, value=name)
        c3 = ws.cell(idx, 3, value=score_rate)
        c4 = ws.cell(idx, 4, value=definition)
        c5 = ws.cell(idx, 5, value=action)
    ...
    # Setup Excel Data Validation for Lv. Column (L1 ~ L5 dropdown)
    ...
    for member in member_names:
        ...
        headers = ["評估面向", "四大文化定義與說明", "部屬自評實例 (STAR)", "評分 (Lv.1-5選單)", "主管回饋 (Feedback)"]
        ...
        for title, ans in culture_rows:
            ...
            c4 = ws.cell(row_idx, 4, value="") # Lv. dropdown
            c5 = ws.cell(row_idx, 5, value="") # Feedback
            
            dv.add(c4) # Attach dropdown validation to cell
        ...
        # 5. 組織卡關點與未來價值展望 Chunk Table
        if se.get("reflection"):
            ws.merge_cells(start_row=row_idx, start_column=1, end_row=row_idx, end_column=5)
            refl_banner = ws.cell(row_idx, 1, value="組織卡關點反思與未來價值展望")
            refl_banner.font = font_sub_header
            refl_banner.alignment = Alignment(horizontal="left", vertical="center", indent=1)
            style_merged_range(ws, row_idx, 1, row_idx, 5, fill=fill_sub_header, border=thin_border)
            ws.row_dimensions[row_idx].height = 24
            row_idx += 1
            
            headers_refl = ["評估面向", "反思與展望項目", "部屬自評回覆內容", "評分 (Lv.1-5選單)", "主管回饋與備註"]
            for col_i, h in enumerate(headers_refl, 1):
                cell = ws.cell(row_idx, col_i, value=h)
                cell.font = font_sub_header
                cell.fill = fill_sub_header
                cell.alignment = align_header
            ...
            end_refl_r = row_idx - 1
            ws.merge_cells(start_row=start_refl_r, start_column=1, end_row=end_refl_r, end_column=1)
            c_refl_cat = ws.cell(start_refl_r, 1, value="卡關與展望")
            c_refl_cat.font = font_body_bold
            c_refl_cat.alignment = align_center
            style_merged_range(ws, start_refl_r, 1, end_refl_r, 1, fill=fill_white, border=thin_border)
...
print("Saved all XLSX with Lv. dropdown validation and full rating guide definitions table!")
