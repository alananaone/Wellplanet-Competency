...
def create_supervisor_chunk_sheet(ws, supervisor_name, member_names, all_entries):
    ...
    # Subtitle Instructions (Merged A2:E2)
    ws.merge_cells("A2:E2")
    sub_cell = ws.cell(2, 1, value="評分標準說明：Lv.1 ~ Lv.5（符合期待 ～ 超越期待） ｜ 填寫方式：主管可於每題右側欄位直接填寫等級評分與 Feedback 回饋")
    sub_cell.font = font_sub_note
    sub_cell.alignment = Alignment(horizontal="left", vertical="center", indent=1)
    style_merged_range(ws, 2, 1, 2, 5, fill=PatternFill(start_color="FFF9F5", end_color="FFF9F5", fill_type="solid"), border=thin_border)
    ...
    for member in member_names:
        ...
        headers = ["評估面向", "四大文化定義與說明", "部屬自評實例 (STAR)", "評分 (Lv.1-5)", "主管回饋 (Feedback)"]
        ...
        if se.get("competencies"):
            ...
            headers_comp = ["評估面向", "職能項目與題目定義", "部屬自評實例", "評分 (Lv.1-5)", "主管回饋 (Feedback)"]
        ...
        if se.get("reflection"):
            ...
            headers_refl = ["評估面向", "反思與展望項目", "部屬自評回覆內容", "評分 (Lv.1-5)", "主管回饋與備註"]
...
print("Saved all XLSX files with cross-column centering, elegant chunk tables, and NO emojis!")
